        ...
        {history.length === 0 ? (
          ...
          history.map((entry) => (
            <motion.div
              key={entry.id}
              layout
              className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 dark:border-slate-800/80 dark:bg-slate-900/60 dark:text-slate-300"
            >
              <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.2em] text-blue-600 dark:text-cyan-200">
                <span>{entry.action}</span>
                <span className="text-slate-500">{entry.timestamp}</span>
              </div>
              <p className="text-[11px] text-slate-500 dark:text-slate-400">{entry.summary}</p>
            </motion.div>
          ))
        )}
      </div>
